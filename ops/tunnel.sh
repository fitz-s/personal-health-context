...
# Runtime key: Keychain item phctx-tunnel-key only (fail closed). Read into this process; never written anywhere.
key() {
  /usr/bin/security find-generic-password -s phctx-tunnel-key -w 2>/dev/null && return
  echo "no runtime key: add it with  security add-generic-password -s phctx-tunnel-key -a phctx -U -w" >&2; exit 3
}
