...
printf '\nCommand: swiftc -typecheck -target arm64-apple-macos14.0 -sdk "$sdk" HealthAuthorizationCoordinator.swift KeychainStore.swift PairingURL.swift\n'
swiftc -typecheck -target arm64-apple-macos14.0 -sdk "$sdk" "$app/HealthAuthorizationCoordinator.swift" "$app/KeychainStore.swift" "$app/PairingURL.swift"
printf 'typecheck exit code: %s\n' "$?"
printf '\nCommand: swiftc -typecheck -target arm64-apple-macos14.0 -sdk "$sdk" -I HealthSyncCore/.build/arm64-apple-macosx/debug/Modules AnchoredSyncCoordinator.swift\n'
swiftc -typecheck -target arm64-apple-macos14.0 -sdk "$sdk" -I "$core/.build/arm64-apple-macosx/debug/Modules" "$app/AnchoredSyncCoordinator.swift"
