...
- **Ingest connection limit** is global (16 concurrent), not per peer: a LAN device holding connections can delay a phone upload until timeouts (~70 s) free slots. LAN-only exposure; the phone retries from its outbox.
