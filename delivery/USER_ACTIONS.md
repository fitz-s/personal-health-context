...
## 1. ChatGPT 连接 — 已完成（2026-09-24）
Secure MCP Tunnel `phctx-mac` 在 launchd 下常驻（`com.personalhealthcontext.tunnel`），ChatGPT 里的 developer-mode app "Personal Health Context" 已连通；写入、新会话读取、照片/PDF 原文件 hash 都实测通过（`live-evidence/chatgpt_live_probe.md`）。按你的指示复用了 WebCodex 现有的 tunnel runtime key（只在进程里读取，没有复制到别处）。
剩下可选：在手机 ChatGPT App 里用同一个 app 试一次（同一连接，无需再配置）。
...
## 3. Apple Health 历史回填 — 已完成
`~/Downloads/export.zip` 已导入生产库（187 万条观测）。以后重新导出时再跑一次同样的命令即可：
```
PYTHONPATH=~/personal-health-context/src ~/personal-health-context/.venv/bin/python -m phctx import-apple ~/Downloads/export.zip
```
Oura 镜像条目会被自动过滤并只计数。这只是历史回填，持续同步要靠第 2 项的 iPhone helper。
